...
**UMCP v2.3.0** · **15,998 tests** · **20 domains** · **193 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 15,998 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**15,998 test cases** across **185 test files** in `tests/` (184 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 15,998 (verify: `pytest --collect-only | grep "::" | wc -l`)

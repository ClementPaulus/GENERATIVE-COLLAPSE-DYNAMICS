...
\begin{itemize}[leftmargin=*,nosep]
  \item \texttt{manifest.json}: provenance metadata---timestamps,
    ...
    unique digest.
  \item \texttt{raw\_data.json}: the raw measurements (here,
    three voltages at 9.9).
  \item \texttt{contract\_ref.yaml}: pointer to the frozen contract
    (\texttt{UMA.INTSTACK.v1}).
  \item \texttt{closure\_registry.yaml}: lists which Tier-2 closures
    are active for this run.
  \item \texttt{expected\_outputs.json}: the embedded trace,
    kernel invariants, and regime classification.
  \item \texttt{ss1m\_receipt.json}: the Single-Source
    Single-Measurement receipt with integrity checksums.
\end{itemize}
...
parameters and produces the same verdict.
...
{"casepack": "casepacks/hello_world",
 "strict": true}
\end{Verbatim}

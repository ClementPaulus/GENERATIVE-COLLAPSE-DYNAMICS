...
Section~\ref{sec:axiom} states the axiom and derives the chain
...
Section~\ref{sec:canon} presents the five-word canon and the
spine---the mandatory five-stop protocol through which every
claim is constructed and audited.
...
\midrule
$\eps$ & $10^{-8}$ & Guard band &
  Unique value where the pole at $\omega = 1$ does not affect
  any measurement to machine precision. \\[0.5em]
$p$ & $3$ & Drift exponent &
  Unique integer where $\omega_{\mathrm{trap}}$ is the Cardano
  root of $x^3 + x - 1 = 0$. No other integer yields a solvable
  algebraic form. \\[0.5em]
$\alpha$ & $1.0$ & Curvature coefficient &
  Unit coupling: $D_C = \alpha \cdot C$.  Seam budget
  normalization. \\[0.5em]
$\lambda$ & $0.2$ & Auxiliary coefficient &
  Return-rate adaptation speed in $\tRS$ dynamics. \\[0.5em]
$\tolseam$ & $0.005$ & Seam tolerance &
  Width where $\IC \leq F$ holds at $100\%$ across all
  18~domains. \\
\bottomrule
\end{tabular}
\end{center}}
...
Symbol / Value & Name & Derivation \\
\midrule
$c^* \approx 0.7822$ & Self-dual fixed point &
  Solves $\ln\!\bigl((1{-}c)/c\bigr) + 1/c = 0$.
  Maximizes $S + \kappa$ per channel. \\[0.5em]
$\omega_{\mathrm{trap}} \approx 0.6823$ & Trapping threshold &
  Solution of $\Gam(\omega) = 1$ where
  $\Gam(\omega) = \omega^3/(1{-}\omega{+}\eps)$.
  Cardano root of $x^3 {+} x {-} 1 = 0$. \\[0.5em]
$c_{\mathrm{trap}} \approx 0.3177$ & Channel trapping &
  $c_{\mathrm{trap}} = 1 - \omega_{\mathrm{trap}}$.
  Duality in channel space. \\[0.5em]
$c_{\mathrm{eq}} = 0.5$ & Equator &
  Geometric center.  $S + \kappa = 0$ exactly.  Quintuple
  fixed point (\S\ref{sec:identities}). \\[0.5em]
$\omega^* \approx 0.218$ & Drift at $c^*$ &
  $\omega^* = 1 - c^*$.  Boundary between coupling growth
  and decay. \\
\bottomrule
\end{tabular}
\end{center}}
...
E1 & Logistic self-duality &
  $c^* = \sigma(1/c^*)$; $c^* \approx 0.7822$ \\
E2 & Coupling maximum &
  $\max(S{+}\kappa) = (1{-}c^*)/c^* \approx 0.278$ \\
E3 & Log-odds reciprocal &
  $\ln\!\bigl(c^*/(1{-}c^*)\bigr) = 1/c^*$ \\
E4 & Integral conservation &
  $\int_0^1 [h(c){+}\ln c]\,dc = -\tfrac{1}{2}$ \\
E5 & Curvature decomposition &
  $f''(c) = -\gF(c) - 1/c^2$ (78\% Fisher + 22\% pole) \\
E6 & Gap identity &
  $\ln 2 - \max(S{+}\kappa) \approx 0.415$ \\
E7 & Fisher-geometric scaling &
  $\Delta$ and $\mathrm{Var}(\arcsin\sqrt{c})$ co-vanish \\
E8 & $\omega$ hierarchy &
  $\omega_\mathrm{s} < \omega^* < \omega_\mathrm{c}
    < \omega_\mathrm{trap}$ \\
\bottomrule
\end{tabular}
\end{center}}
...
Tag & Name & Statement \\
\midrule
B1 & Pythagorean duality &
  $F + \omega = 1$ \\
B2 & Integrity bound &
  $\IC \leq F$; equality iff homogeneous \\
B3 & Composition law &
  $\IC_{12} = \sqrt{\IC_1 \cdot \IC_2}$;
  $F_{12} = (F_1{+}F_2)/2$ \\
B4 & Fisher volume &
  $Z = -\sum w_i \ln[c_i(1{-}c_i)]$ \\
B5 & Budget conservation &
  $\Delta\kappa = R \cdot \tR - (D_\omega{+}D_C)$;
  $|\Delta\kappa| \leq \tolseam$ \\
B6 & Self-dual maximum &
  $f(c^*) = 2\cos^2\theta^* \cdot \ln(\tan\theta^*)$ \\
B7 & Cubic trapping &
  $\Gam(\omega_\mathrm{trap}) = 1$; root of
  $x^3{+}x{-}1 = 0$ \\
B8 & Curvature = Fisher + pole &
  $f''(c) = -1/[c(1{-}c)] - 1/c^2$ \\
B9 & Low-rank closures &
  5 diagnostics $\to$ 4 effective dim
  (Thm.~\ref{thm:dof}) \\
B10 & Geodesic partition &
  $\{\eps, c_\mathrm{trap}, \tfrac{1}{2}, c^*, 1{-}\eps\}$
  partition $[0,\pi]$ \\
B11 & Cost elasticity &
  $\varepsilon_\Gam = \omega(3{-}2\omega)/(1{-}\omega)
  \to 4$ at pole \\
B12 & IC democracy &
  $\mathrm{CV}(\text{IC-drop}) \approx 7 \times 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{center}}
...
\midrule
D1 & Fisher flatness &
  $\gF(\theta) = 1$; Bernoulli manifold is flat \\
D2 & $f$ in Fisher coords &
  $f(\theta) = 2\cos^2\theta \cdot \ln(\tan\theta)$;
  verified $< 10^{-16}$ \\
D3 & $\kappa$ as log-sine &
  $\kappa = 2\sum w_i \ln|\sin\theta_i|$ \\
D4 & $n{=}2$ determinancy &
  $c_{1,2} = F \pm \sqrt{F^2 - \IC^2}$;
  real iff $\IC \leq F$ \\
D5 & $p{=}3$ Cardano root &
  Discriminant $D = -31 < 0$; unique real root \\
D6 & Composition invariance &
  $\Delta(n\,\mathrm{copies}) = \Delta(1)$ exactly \\
D7 & Regime partition &
  Collapse 63\% / Watch 24\% / Stable 12.5\% \\
D8 & Dimension collapse &
  8-channel kernel $\to$ 4 effective dim \\
\bottomrule
\end{tabular}
\end{center}}
...
Tag & Name & Statement \\
\midrule
N1 & Fisher-entropy integral &
  $\int_0^1 \gF \cdot S\,dc = \pi^2/3 = 2\zeta(2)$ \\
N2 & Coupling centroid &
  $\int_0^1 (S{+}\kappa)\,c\,dc = 0$ \\
N3 & Rank-2 closed form &
  $\IC = \sqrt{F^2 - C^2/4}$ for $n = 2$ \\
N4 & Equator quintuple &
  At $c = \tfrac{1}{2}$: $S = \ln 2$, $S{+}\kappa = 0$,
  $h' = 0$, $\gF = 4$, $\theta = \pi/4$ \\
N5 & Fano-Fisher duality &
  $h''(c) = -\gF(c) = -1/[c(1{-}c)]$ \\
N6 & Triple peak identity &
  $(1{-}c^*)/c^* = e^{-1/c^*} = (S{+}\kappa)|_{c^*}$ \\
N7 & Asymptotic IC-curvature &
  $\IC^2 \approx F^2 - \beta_n C^2$ \\
N8 & Log-integrity correction &
  $\kappa = \ln F - C^2/(8F^2) + O(C^4)$ \\
N9 & Budget cost crossover &
  $\exists\,\omega_\mathrm{cross} \approx 0.58$:
  $\Gam(\omega) = E[D_C|\omega]$ \\
N10 & Jensen entropy bound &
  $S \leq h(F)$; equality iff $C = 0$ \\
N11 & Moment family &
  $\mu_n =
  \frac{(n{+}1)H_{n{+}1}{-}(n{+}2)}{(n{+}1)^2(n{+}2)}$ \\
N12 & Gap composition &
  $\Delta_{12} = (\Delta_1{+}\Delta_2)/2$
  ${} + (\sqrt{\IC_1}{-}\sqrt{\IC_2})^2\!/2$ \\
N13 & Entropy moment table &
  $\int_0^1 c^n S\,dc$ has closed rational form \\
N14 & Jensen entropy-fidelity &
  $S \leq h(F)$ (formalized sandwich) \\
N15 & Gap Taylor &
  $\Delta \approx C^2/(8F)$ \\
N16 & Reflection formula &
  $f(\theta) + f(\pi/2{-}\theta)
  = 2\ln(\tan\theta)\cos(2\theta)$ \\
\bottomrule
\end{tabular}
\end{center}}
...
L2 & $\omega$ range &
  $\omega \in [\eps, 1{-}\eps]$ \\
L3 & $\kappa$ bounds &
  $\kappa \in [\ln\eps, 0] \approx [-18.42, 0]$ \\
L4 & IC = geometric mean &
  $\IC = \exp(\sum w_i \ln c_i)$ \\
L5 & $S = \ln 2$ iff equator &
  $S(c) = \ln 2 \Leftrightarrow c = 1/2$ \\
L6 & IC sensitivity &
  $\partial\IC/\partial c_i = (w_i/c_i)\IC$ \\
L7 & $\IC \leq F$ &
  Integrity bound on $\eps$-clamped domain \\
L8 & Regime well-defined &
  Every $(\omega,F,S,C) \mapsto$ one regime \\
L9 & Stable $\subset$ (low $\omega$, high $F$) &
  Stable requires $\omega < 0.038$ and $F > 0.90$ \\
L10 & Lipschitz $F$ &
  $|\Delta F| \leq \max(w_i) \cdot \|\Delta c\|_\infty$ \\
L11 & Lipschitz $\kappa$ &
  $|\Delta\kappa| \leq \max(w_i/c_i) \cdot \|\Delta c\|_\infty$ \\
L12 & $C$ monotone in spread &
  Wider $c_i$ spread $\to$ larger $C$ \\
L13 & $S$ Lipschitz &
  Bounded by $\max(w_i) \cdot |\ln((1{-}c_{\min})/c_{\min})|$ \\
L14 & $F$ monotone up &
  $F$ non-decreasing in each $c_i$ \\
L15 & $\kappa$ monotone up &
  $\kappa$ non-decreasing in each $c_i$ \\
L16 & $\IC$ monotone up &
  $\IC$ non-decreasing in each $c_i$ \\
L17 & $\eps$-clipping bounded &
  $|F_\mathrm{clip} - F_\mathrm{raw}|
  \leq \max(w_i)\eps$ \\
L18 & $\kappa$ clipping bounded &
  $|\kappa_\mathrm{clip} - \kappa_\mathrm{raw}|$ bounded \\
L19 & Stable $\Rightarrow$ low budget &
  $\Gam(\omega) < 10^{-5}$ in Stable \\
L20 & Seam telescoping &
  $\sum_i \Delta\kappa_i = \kappa(t_n) - \kappa(t_0)$ \\
L21 & Residual telescoping &
  Cumulative residual = $\sum$ budget $-$ $\sum$ ledger \\
L22 & $\Gam$ convexity &
  $\Gam$ convex on $[0, 1{-}\eps)$ for $p \geq 2$ \\
L23 & $\Gam$ pole at $\omega = 1$ &
  $\lim_{\omega\to1}\Gam(\omega) = +\infty$ \\
L24 & Stable attractor &
  Near-stable states require $O(\eps^2)$ to exit \\
L25 & Return monotonicity &
  If $\tR < \infty$ and $\omega$ decreases:
  $\tR$ can only decrease \\
L26 & Weld strengthening &
  Adding closures tightens, never loosens \\
L27 & Residual growth bound &
  $|\text{cumul.}| \leq K \cdot \tolseam$ for $K$ seams \\
L28 & Closure minimality &
  Removing redundancy preserves weld verdict \\
L29 & Contract monotonicity &
  Tightening preserves PASS if data unchanged \\
L30 & IC collapse cascade &
  $c_i \to \eps \Longrightarrow
  \IC \to \eps^{w_i} \cdot \IC_\mathrm{rest}$ \\
L31 & Zero-weight pruning &
  $w_i = 0$ channels deletable without change \\
L32 & Weight Lipschitz &
  $|\Delta F| \leq \max(c_i) \cdot \|\Delta w\|_1$ \\
L33 & IC/$F$ ratio bound &
  $\IC/F \in [\eps^{(1-\min w_i)}/F, \, 1]$ \\
L34 & Entropy-curvature coupling &
  $C \to 0 \Longrightarrow S \to h(F)$, $\IC \to F$ \\
\bottomrule
\end{tabular}
\end{center}}
...
\# & Name & Statement \\
\midrule
L35 & Return-collapse duality &
  If $\tR$ finite and drift-dominated: $\tR \approx D_C$ \\
L36 & Generative flux bound &
  $\Phi_\mathrm{gen} \leq \Gam(\omega)(1{-}S)$ \\
L37 & Duality-horizon transition &
  $\Delta\kappa \approx 0.1$ marks cost-credit boundary \\
L38 & Universal horizon deficit &
  Cross-domain mean $\IC$ at deficit $\approx 0.947$ \\
L39 & Super-exp.\ convergence &
  IC convergence to $F$ faster than exponential as
  $C \to 0$ \\
L40 & Stable attractor radius &
  $\IC/F > 0.95$, $\omega < 0.05$ basin width \\
L41 & Entropy-integrity anti-corr. &
  $S + \kappa = 0$ at $c = 1/2$;
  $\max(S{+}\kappa)$ at $c^*$ \\
L42 & Coherence-entropy product &
  $\IC \cdot e^S \leq 1$; equality at $c = 1/2$ \\
L43 & RCFT convergence &
  Recursive kernel application converges \\
L44 & Fractal return scaling &
  Return distances: power law in recursion depth \\
L45 & Seam residual algebra &
  Monoid; associativity error
  $5.55 \times 10^{-17}$ \\
L46 & Weld closure composition &
  PASS$_1 \wedge$ PASS$_2 \Rightarrow$ PASS$_{12}$
  if residuals within tolerance \\
\bottomrule
\end{tabular}
\end{center}}
...
{\small
\begin{center}
\begin{tabular}{@{}P{2.1cm}P{2.1cm}P{2.8cm}@{}}
\toprule
GCD structure & Removed structure & Classical limit \\
\midrule
$F + \omega = 1$ &
  Fisher geometry, collapse field &
  Unitarity \\[0.3em]
$\IC \leq F$ &
  Solvability, composition laws, gap diagnostic &
  Weighted AM-GM \\[0.3em]
$S$ (Bernoulli) &
  Intermediate values of collapse field &
  Shannon entropy \\[0.3em]
$\IC = e^\kappa$ &
  Channel weights, measurement context &
  Exponential map \\[0.3em]
Regime gates &
  Third verdict, non-ergodic switching &
  Binary pass/fail \\[0.3em]
Budget $\Delta\kappa$ &
  Epistemic cost, return typing &
  Energy conservation \\
\bottomrule
\end{tabular}
\end{center}}
...
\quad Gap diagnostic & \checkmark & --- \\[0.3em]
...
\quad Entropy--fidelity bound & \checkmark & --- \\[0.3em]
...
\midrule
\conform{} &
  All identity checks pass, schema conforms, SHA-256 integrity
  verified, regime computed, seam residual within tolerance. \\[0.3em]
\nonconform{} &
  One or more checks fail with a definite violation. \\[0.3em]
\regime{non\_evaluable} &
  The data, contract, or closure is insufficient to produce a
  verdict.  This is not failure---it is the recognition that the
  question cannot be answered with the available structure. \\
\bottomrule
\end{tabular}
\end{center}
...
Drift & \textit{derivatio} &
  Debit $D_\omega$ to the ledger.
  $\omega = 1 - F$: what moved relative to the contract. \\[0.3em]
...
Roughness & \textit{curvatura} &
  Debit $D_C$ to the ledger.
  $C = \mathrm{std}(\tvec)/0.5$: where it was bumpy. \\[0.3em]
Return & \textit{reditus} &
  Credit $R \cdot \tR$ in the ledger.
...
\textbf{Canon} &
  Narrate using the five words: Drift, Fidelity, Roughness,
  Return, Integrity. \\[0.3em]
\textbf{Closures} &
  Publish thresholds; stance must change when gates are
  crossed.  No mid-episode edits. \\[0.3em]
\textbf{Integrity Ledger} &
  Debit Drift + Roughness, credit Return.  Residual must
  close: $|s| \leq \tolseam$. \\[0.3em]
\textbf{Stance} &
  Derived verdict: \regime{Stable} / \regime{Watch} /
  \regime{Collapse}.  Never asserted. \\
\bottomrule
\end{tabular}
\end{center}
